...
- **2026-08-06** — `fetchSerieMonetaria` pedía un único request con `limit: 3000` para traer "todo" el historial de una variable del BCRA. La IA asumió que 3000 alcanzaba sin verificarlo contra el total real disponible — no falló nunca, ni tiró ningún error: simplemente devolvía en silencio los ~3000 registros más recientes de cada serie. Para inflación (≈1000 registros mensuales desde 1943) esto no tenía consecuencia, pero para tipo de cambio (5986 registros diarios, historial real desde 2002) y tasa de depósitos (9586 registros diarios, historial real desde 1985) recortaba la información disponible a aproximadamente 2014 en adelante, sin ningún indicio visible del recorte. Este error **no lo encontró la IA**: lo notó Santiago al intentar elegir una fecha anterior a 2010 en la calculadora y no poder. Se corrigió agregando `fetchSerieMonetariaCompleta`, que pagina con el parámetro `offset` de la API hasta recibir una página más corta que el límite (señal de que no queda más historial). Ver decisión 014 en `DECISIONS.md`.

- **2026-08-07** — Al refactorizar `AjusteAlquileres.tsx` para usar `useAsyncData`, la IA escribió la lógica de "calcular las fechas por default cuando llegan los datos" dentro de un `useMemo` que llamaba a `setState` (`setMesOrigen`/`setMesDestino`) en su cuerpo. Esto es incorrecto en React: `useMemo` debe ser una función pura que calcula un valor, no un lugar para efectos secundarios como actualizar estado — React puede llamar a la función de `useMemo` más de una vez o descartar el resultado sin avisar, lo cual puede duplicar actualizaciones de estado o comportarse de forma impredecible bajo versiones concurrentes de React. El error no llegó a `tsc` ni a un test: lo notó la IA misma al releer el código que acababa de escribir, antes de correr nada. Se corrigió cambiando `useMemo` por `useEffect`, que sí es el hook correcto para "efecto secundario cuando cambia una dependencia".

- **2026-08-07** — Al escribir la primera suite de tests de componentes con React Testing Library, dos tests fallaron con síntomas confusos: un mock contado "3 veces" en vez de 2, y "múltiples elementos encontrados" para un texto que debería aparecer una sola vez. La IA investigó la causa real en vez de ajustar las aserciones para que pasaran: sin `afterEach(cleanup)`, los componentes renderizados en un test seguían montados en el DOM durante el siguiente test del mismo archivo (este proyecto no usa los globals de Vitest, así que la limpieza automática de `@testing-library/react` nunca se activaba); y sin `vi.restoreAllMocks()`, un `vi.spyOn` de un test arrastraba su historial de llamadas al siguiente test que espiaba la misma función. Ambos se corrigieron una sola vez en `src/test-setup.ts` (que corre antes de cada archivo de test), no test por test. Ver decisión 016 en `DECISIONS.md`.
