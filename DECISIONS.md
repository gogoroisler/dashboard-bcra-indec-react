...
### 015 — Hook `useAsyncData` compartido, con reintentar manual y validación de montos
**Fecha:** 2026-08-07
**Decisión:** Se extrae `src/hooks/useAsyncData.ts`, que centraliza el patrón repetido en los 4 componentes de datos (fetch al montar, estado de carga/error, limpieza si el componente se desmonta antes de resolver) y agrega algo que no existía: un `reintentar()` manual, sin recargar toda la página. Los 4 componentes pasan a usarlo, junto con un componente `MensajeError` compartido (mensaje + botón "Reintentar"). Además, las tres calculadoras (`ComparadorInversion`, `EvolucionSalarial`, `AjusteAlquileres`) ahora validan que los montos ingresados sean números mayores a cero — antes, un campo vacío se interpretaba como `$0` en silencio (`Number('')` es `0`, no `NaN`, así que el chequeo existente de `Number.isNaN` no lo detectaba) y un monto negativo se calculaba sin ningún aviso.
**Por qué:** Antes de esta sesión, la única verificación de errores era manual (capturas de pantalla). Al auditar los 4 componentes con la intención de escribir tests, aparecieron gaps reales de UX: sin forma de reintentar un fetch fallido salvo refrescar la página entera, y montos inválidos que se calculaban en silencio en vez de avisar. Extraer el hook antes de escribir los tests también los hace más simples: se testea el fetch/carga/error/reintentar una sola vez (en el hook), no cuatro veces (una por componente).
**Alternativas consideradas:** dejar el fetch inline en cada componente y solo agregar el botón de reintentar cuatro veces — descartado por duplicar exactamente la misma lógica de carga/error/cancelado que ya existía cuatro veces.

---

### 016 — Vitest + React Testing Library para tests de componentes
**Fecha:** 2026-08-07
**Decisión:** Se suman `@testing-library/react`, `@testing-library/jest-dom` y `@testing-library/user-event`, con `jsdom` como entorno vía el comentario `// @vitest-environment jsdom` en cada archivo de test de componente (el entorno global sigue siendo `node`, más rápido, para los tests de `src/lib/`). Se testea: el hook `useAsyncData` completo (carga, error, reintentar), el componente `MensajeError`, y de las tres calculadoras solo `AjusteAlquileres` en profundidad (la más simple, cubre carga/error/reintentar/validación de monto — no tiene sentido repetir exactamente lo mismo en las otras dos, que comparten el mismo hook). Para `ComparadorInversion` se agrega un único test dirigido a su comportamiento distintivo: que IPCBA falle con su propio error sin romper el resto de las tarjetas cuando su serie no cubre el mes elegido.
**Por qué:** El objetivo no es cobertura por cobertura — es testear lo que realmente puede fallar. `AjusteAlquileres` ya prueba el patrón compartido (hook + validación); duplicarlo en las otras dos calculadoras no agrega información nueva. El caso de IPCBA sí es información nueva: es la única lógica de "una fuente falla, las demás no" que existe en el proyecto.
**Bugs de aislamiento entre tests encontrados al escribir esta suite:** (1) sin `afterEach(cleanup)`, los `render()` de tests distintos del mismo archivo se acumulaban en el DOM en vez de reemplazarse — como este proyecto no usa los globals de Vitest (`test.globals`), `@testing-library/react` no registra su limpieza automática. (2) sin `vi.restoreAllMocks()`, un `vi.spyOn` de un test arrastraba su historial de llamadas (`mock.calls`) al siguiente test que espiaba la misma función, dando conteos de llamadas inflados. Ambos se corrigieron en `src/test-setup.ts`, que corre antes de cada archivo de test.
